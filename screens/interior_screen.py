...
class InteriorScreen(QWidget):
    ...
    def __init__(self, interior_space: InteriorSpace, available_modules: list[Module], environment=None, center=None):
        ...
        # Splitter to divide the interior space draw and the subspace editors
        ...
        # Draw the interior space and subspaces
        self._draw_space()
    ...
    def set_environment(self, environment):
        ...
        if environment:
            try:
                ...
                self.interior_space.resize(size_x, size_y)
            except Exception:
                pass
        self._draw_space()
    ...
    def _add_subspace(self):
        ...
        # Check if it fits within the interior space boundaries
        if (new_x + new_subspace.size_x <= self.interior_space.get_size_x() and
            ...
            # Add to data structure, redraw interior, and add editor
            self.interior_space.add_subspace(new_subspace)
            self._draw_space() # Redraw interior to show the new subspace
    ...
    def distancia_entre_center_y_modulo(self, modulo):
        """
        Devuelve la distancia entre el Center y un módulo (en el interior), usando el Environment si está disponible.
        """
    ...
    def _delete_subspace(self, subspace, subspace_editor_group):
        """Delete a subspace from the interior space."""
        ...
        if confirm == QMessageBox.Yes:
            # Remove the subspace from the interior space
            self.interior_space.remove_subspace(subspace)
